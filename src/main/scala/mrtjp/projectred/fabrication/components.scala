...
object ICComponentStore {
  ...
  def registerIcons(reg: IIconRegister) {
    ...
    for (i <- 0 until 16)
      insulatedwireIcons(i) = register("insulatedwire/conn" + i)
    for (i <- 0 until 16)
      bundledwireIcons(i) = register("bundledcable/conn" + i)
  ...
  def orthoGridT(xSize: Double, ySize: Double) =
    new Scale(xSize, 1, -ySize) `with` new Rotation(
      0.5 * MathHelper.pi,
      1,
      0,
      0
  ...
  def orthoPartT(
    ...
    new TransformationList(
      ...
      new Translation(
        new Vector3(xPoint, 0, yPoint)
          .multiply(1.0 / boardSize.width, 0, 1.0 / boardSize.height)
      ),
...
abstract class SingleComponentModel(pos: Vector3 = Vector3.zero)
    extends ICComponentModel {
  val models = faceModels.map(
    _.copy
      .apply(new Translation(-0.5, 0, -0.5))
      .apply(pos.copy.multiply(1 / 16d).translation)
  )
...
abstract class CenteredSingleComponentModel extends ICComponentModel {
  ...
  override def renderModel(t: Transformation, orient: Int, ortho: Boolean) {
    faceModels(dynamicIdx(orient, ortho))
      .render(dynamicT(orient) `with` t, getUVT)
  }
}

abstract class OnOffModel(pos: Vector3 = Vector3.zero)
    extends SingleComponentModel(pos) {
  var on = false
...
class BaseComponentModel(val iconPath: String)
    extends CenteredSingleComponentModel {
...
class RedstoneTorchModel(x: Double, z: Double)
    extends OnOffModel(new Vector3(x, 0, z)) {
...
class WireModel(val iconPath: String) extends ICComponentModel {
  ...
  override def renderModel(t: Transformation, orient: Int, ortho: Boolean) {
    faceModels(dynamicIdx(orient, ortho)).render(
      dynamicT(orient) `with` t,
      new IconTransformation(icon),
      ColourMultiplier.instance(
        if (disabled) disabledColour else if (on) onColour else offColour
      )
    )
  }
}
...
class IOSigModel extends ICComponentModel {
  ...
  override def renderModel(t: Transformation, orient: Int, ortho: Boolean) {
    ...
    m.render(
      t0,
      new IconTransformation(ioBorder),
...
class PointerModel(x: Double, z: Double, scale: Double = 1)
    extends ICComponentModel {
  val pos = new Vector3(x, 0, z).multiply(1 / 16d)
  val models = faceModels.map(
    _.copy
      .apply(new Translation(-0.5, 0, -0.5))
      .apply(new Scale(scale, 1, scale))
  )

  var angle = 0.0

  override def renderModel(t: Transformation, orient: Int, ortho: Boolean) {
    models(dynamicIdx(orient, ortho)).render(
      new Rotation(-angle, 0, 1, 0) `with` pos.translation
