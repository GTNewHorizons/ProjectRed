...
object RenderHalo {
  ...
  private class LightCache(
      ...
      val color: Int,
      val cube: Cuboid6
  ) extends Ordered[LightCache] {
    def this(x: Int, y: Int, z: Int, c: Int, cube: Cuboid6) =
      this(new BlockCoord(x, y, z), c, cube)

    private def renderDist =
      vec.set(pos.x, pos.y, pos.z).sub(renderEntityPos).magSquared
  ...
  def onRenderWorldLast(event: RenderWorldLastEvent) {
    ...
    val max =
      if (Configurator.lightHaloMax < 0) renderList.size
      else Configurator.lightHaloMax
  ...
  private def renderHalo(world: World, cc: LightCache) {
    CCRenderState.instance.setBrightnessInstance(
      world,
      cc.pos.x,
      cc.pos.y,
      cc.pos.z
