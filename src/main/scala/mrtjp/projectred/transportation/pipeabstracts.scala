...
abstract class SubcorePipePart
    extends TMultiPart
  ...
  override def getSubParts = {
    ...
    for (s <- 0 until 6)
      if (maskConnects(s)) boxes :+= new IndexedCuboid6(s, oBounds(s))
    boxes
  }
...
trait TPipeTravelConditions {
  ...
  /** 00FT T - can travel to F - can come from
    */
...
abstract class PayloadPipePart[T <: AbstractPipePayload]
    extends SubcorePipePart
    with TPipeTravelConditions {
  ...
  def pushItemFlow() {
    ...
    for (r <- itemFlow.it)
      if (r.isCorrupted) itemFlow.scheduleRemoval(r)
      else {
  ...
  def chooseRandomDestination(r: T, mask: Int) {
    var moves = Seq[Int]()
    for (i <- 0 until 6)
      if ((connMap & 1 << i) != 0 && i != (r.input ^ 1) && (mask & 1 << i) == 0)
        moves :+= i
  ...
  override def onNeighborChanged() {
    ...
    if (connCount == 0)
      if (!world.isRemote)
        for (r <- itemFlow.it) if (itemFlow.scheduleRemoval(r)) {
          r.preItemRemove()
          world.spawnEntityInWorld(r.getEntityForDrop(x, y, z))
        }
  }
