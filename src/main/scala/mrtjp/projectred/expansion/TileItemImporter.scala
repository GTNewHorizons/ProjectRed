...
class TileItemImporter
  ...
  // side = out, side^1 = in
  override def canAcceptInput(item: ItemKey, side: Int) =
    (side ^ 1) == this.side && !powered && storage.isEmpty
  ...
  def suckEntities(box: Cuboid6): Boolean = {
    ...
    for (ei <- elist)
      if (
        !ei.isDead && ei.getEntityItem.stackSize > 0 && canImport(
          ItemKey.get(ei.getEntityItem)
        )
      ) {
  ...
  def canSuckEntities: Boolean = {
    ...
    world.isAirBlock(bc.x, bc.y, bc.z) || !world
      .getBlock(bc.x, bc.y, bc.z)
