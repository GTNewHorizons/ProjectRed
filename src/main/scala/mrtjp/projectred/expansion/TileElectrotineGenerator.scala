...
class TileElectrotineGenerator
  ...
  def getBurnTimeScaled(i: Int) =
    math.min(i, i * burnTimeRemaining / getBurnTimePerDust)
...
class ContainerElectrotineGenerator(
    p: EntityPlayer,
    tile: TileElectrotineGenerator
...
class GuiElectrotineGenerator(
    tile: TileElectrotineGenerator,
    c: ContainerElectrotineGenerator
) extends NodeGui(c, 176, 171) {
  override def drawBack_Impl(mouse: Point, frame: Float) {
    ...
    if (
      tile.cond.charge < tile.getDrawFloor && (tile.storage > 0 || tile.burnTimeRemaining > tile.getBurnUseOnCharge)
    )
      GuiDraw.drawTexturedModalRect(30, 46, 211, 0, 23, 9)

    if (
      tile.burnTimeRemaining > tile.getBurnUseOnCharge && tile.storage < tile.getMaxStorage
    )
  ...
  override def buildGui(player: EntityPlayer, data: MCDataInput) = {
    WorldLib.getTileEntity(player.worldObj, data.readCoord()) match {
      case t: TileElectrotineGenerator =>
        new GuiElectrotineGenerator(t, t.createContainer(player))
      case _ => null
    }
  }
}
...
object RenderElectrotineGenerator extends TCubeMapRender {
  ...
  override def getData(w: IBlockAccess, x: Int, y: Int, z: Int) = {
    val te =
      WorldLib.getTileEntity(w, x, y, z, classOf[TileElectrotineGenerator])
  ...
  override def registerIcons(reg: IIconRegister) {
    ...
    iconT1 =
      new MultiIconTransformation(bottom, top, side1, side2a, side1, side1)
    iconT2 =
      new MultiIconTransformation(bottom, top, side1, side2b, side1, side1)
    iconT3 =
      new MultiIconTransformation(bottom, top, side1, side2c, side1, side1)
    iconT4 =
      new MultiIconTransformation(bottom, top, side1, side2d, side1, side1)
  }
}
