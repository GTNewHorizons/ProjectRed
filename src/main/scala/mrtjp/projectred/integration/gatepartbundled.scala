...
import mrtjp.projectred.transmission.{
  APIImpl_Transmission,
  TFaceBundledAquisitions
}
...
trait TBundledGatePart
    extends GatePart
    with TFaceBundledAquisitions
    with IBundledEmitter {
  ...
  abstract override def discoverStraightOverride(absDir: Int) = {
    ...
    WorldLib.getTileEntity(world, pos) match {
      ...
      case _
          if APIImpl_Transmission.canConnectBundled(world, pos, absDir ^ 1) =>
        true
  ...
  override def getBundledSignal(r: Int) = {
    ...
    if ((logic.bundledOutputMask(shape) & 1 << ir) != 0)
      logic.getBundledOutput(this, ir)
    else null
  }
...
trait TBundledGateLogic[T <: TBundledGatePart] extends GateLogic[T] {
  abstract override def canConnectTo(gate: T, part: IConnectable, r: Int) =
    part match {
      case be: IBundledEmitter => canConnectBundled(gate, r)
      case _                   => super.canConnectTo(gate, part, r)
    }

  def canConnectBundled(gate: T, r: Int): Boolean =
    canConnectBundled(gate.shape, r)
  def canConnectBundled(shape: Int, r: Int): Boolean =
    ((bundledInputMask(shape) | bundledOutputMask(shape)) & 1 << r) != 0
...
class BundledGatePart
    extends RedstoneGatePart
    with TBundledGatePart
    with TComplexGatePart {
...
abstract class BundledGateLogic(val gate: BundledGatePart)
    extends RedstoneGateLogic[BundledGatePart]
    with TBundledGateLogic[BundledGatePart]
    with TComplexGateLogic[BundledGatePart]
...
class BusTransceiver(gate: BundledGatePart) extends BundledGateLogic(gate) {
  ...
  override def getBundledOutput(gate: BundledGatePart, r: Int) =
    if (r == 0) output0 else output2

  def getBundledInput(r: Int) = raiseSignal(
    copySignal(gate.getBundledInput(r)),
    getBundledOutput(gate, r)
  ) // OR'd w/ output
  ...
  override def onChange(gate: BundledGatePart) {
    ...
    if (
      !signalsEqual(output0, getBundledOutput(0)) || !signalsEqual(
        output2,
        getBundledOutput(2)
      )
    ) gate.scheduleTick(2)
...
class BusRandomizer(gate: BundledGatePart) extends BundledGateLogic(gate) {
  ...
  override def scheduledTick(gate: BundledGatePart) {
    ...
    output =
      if ((gate.state & 0xf) != 0)
        if (gate.shape == 0) calc1BitOut else calcNBitOut
      else oldOut
    if (oldOut != output) {
      unpackedOut = unpackDigital(unpackedOut, output)
  ...
  def calcNBitOut = {
    var out = 0
    for (i <- 0 until 16)
      if ((mask & 1 << i) != 0 && rand.nextBoolean) out |= 1 << i
    out
  }

  override def getBundledOutput(gate: BundledGatePart, r: Int) =
    if (r == 0) unpackedOut else null
...
class BusConverter(gate: BundledGatePart) extends BundledGateLogic(gate) {
  ...
  def packClientData =
    rsIn | rsOut << 4 | mostSignificantBit(bIn) << 8 | mostSignificantBit(
      bOut
    ) << 12
  ...
  override def getOutput(gate: BundledGatePart, r: Int) =
    if (gate.shape != 0 && r == 2) rsOut
    else if ((gate.state & 0x10 << r) != 0) 15
    else 0

  override def getBundledOutput(gate: BundledGatePart, r: Int) =
    if (gate.shape == 0 && r == 0) bOutUnpacked else null
  ...
  /** Shape 0: Analog to Digital (RS to Bundled) Shape 1: Digital to Analog
    * (Bundled to RS)
    */
  override def cycleShape(gate: BundledGatePart) = {
    gate.setShape(gate.shape ^ 1)
...
class BusInputPanel(gate: BundledGatePart) extends BundledGateLogic(gate) {
  ...
  override def getSubParts(gate: BundledGatePart) = (0 until 16).map(i =>
    new IndexedCuboid6(
      i,
      (if ((pressMask & 1 << i) != 0) pressed else unpressed) (i).copy
        .apply(VecLib.orientT(gate.orientation))
    )
  )

  override def activate(
      part: BundledGatePart,
      player: EntityPlayer,
      held: ItemStack,
