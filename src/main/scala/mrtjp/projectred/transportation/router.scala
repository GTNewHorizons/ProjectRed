  ...
  def getOrCreateRouter(uu: UUID, holder: IWorldRouter): Router = {
    routers synchronized {
      ...
      if (routers.length <= newLease)
        while (routers.length <= (newLease * 1.5).asInstanceOf[Int] + 1)
          routers :+= null
...
class Router(ID: UUID, parent: IWorldRouter) extends Ordered[Router] {
  ...
  /** Vector, indexed by IP, of Vector of all paths to said IP ordered by
    * distance *
    */
  private var routeTable = Vector[Vector[StartEndPath]]()

  /** Vector of [StartEndPath] sorted by distance, closest one having the lowest
    * index *
    */
  ...
  private def updateLSAIfNeeded(): Boolean = {
    ...
    if (adjacentChanged) {
      ...
      routedExits =
        newAdjacent.foldLeft(0)((b, p) => (b | 1 << p.hopDir) & 0x3f)

      Router.LSADatabasewriteLock.lock()
      LSA.neighbors =
        adjacentLinks // add to database so another thread can make the route table.
  ...
  def decommission() {
    Router.LSADatabasewriteLock.lock()
    if (Router.LSADatabase.isDefinedAt(IPAddress))
      Router.LSADatabase(IPAddress) = null
    Router.LSADatabasewriteLock.unlock()
  ...
  def isInNetwork(destination: Int): Boolean = {
    ...
    if (
      rt.isDefinedAt(destination) && RouterServices.routerExists(destination)
    ) {
  ...
  def canRouteTo(destination: Int, item: ItemKey, priority: NetworkPriority) =
    pathTo(destination, item, priority) != null
  def getDirection(destination: Int, item: ItemKey, priority: NetworkPriority) =
    pathTo(destination, item, priority) match {
  ...
  def refreshRoutingTable(newVer: Int) {
    ...
    var routersByCost2 =
      Vector[StartEndPath](new StartEndPath(this, this, 6, 0))

    /** Queue of all paths that need checking * */
    val openPaths = new JPriorityQueue[StartEndPath](
      Math.sqrt(sizeEstimate).toInt
    ) // scala PQ is not working ?!

    /** Filters of previously found paths * */
    var closedFilters =
      new Array[Vector[Set[PathFilter]]](Router.getEndOfIPPool + 1)
    def ensureClosed(size: Int) {
      while (closedFilters.length <= size) closedFilters :+= null
    }
    ...
    while (!openPaths.isEmpty) breakable {
      ...
      if (lsa != null) {
        for (p <- lsa.neighbors)
          if ((pflags & p.pathFlags) != 0) openPaths.add(dequeue --> p)
      }

      // Approve this candidate
      if ((pflags & 0x3) != 0)
        routersByCost2 :+= dequeue // if we can get to or come from using this path its a possibe route
      closedFilters(deqIP) =
    ...
    def ensureRT2(size: Int) {
      while (routeTable2.length <= size) routeTable2 :+= null
    }
  ...
  def isLoaded = {
    if (
      ...
      parent.getContainer.tile.isInvalid
    ) false
    else true
  }
