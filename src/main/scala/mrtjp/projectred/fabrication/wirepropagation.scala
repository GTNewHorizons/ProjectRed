...
import mrtjp.projectred.fabrication.circuitparts.{CircuitPart, IWireICPart}
...
object ICPropagator {
  ...
  def propagateTo(part: IWireICPart, from: CircuitPart, mode: Int) {
    var p = currentRun
    if (p == null)
      p = if (reusableRuns.isEmpty) new ICPropagationRun else reusableRuns.pop
...
class ICPropagationRun {
  ...
  def addAnalogDrop(part: IWireICPart) {
    analogDrops += new ICPropagation(
      part,
      ICPropagator.notApart,
      IWireICPart.RISING
    )
    aChange = true
  }
