...
trait TNetworkTravelConditions extends TPipeTravelConditions {
  ...
  /** 0CBR R - allow Routing B - allow Broadcastint C - allow Crafting
    */
...
trait TNetworkPipe
    extends PayloadPipePart[NetworkPayload]
    with TInventoryPipe[NetworkPayload]
    with IWorldRouter
    with TRouteLayer
    with IWorldRequester
    with TNetworkTravelConditions
    with TNetworkSubsystem {
  var searchDelay = {
  ...
  private def handleLinkMap(packet: MCDataInput) {
    ...
    for (i <- 0 until 6) {
      if ((high & 1 << i) != 0)
        RouteFX2.spawnType3(RouteFX2.color_linked, i, this)
      if ((low & 1 << i) != 0)
        RouteFX2.spawnType2(RouteFX2.color_unlinked, i, this)
    }
  ...
  final abstract override def update() {
    ...
    if (!world.isRemote)
      getRouter.update(
        world.getTotalWorldTime % Configurator.detectionFrequency == searchDelay
      )
    super.update()
  ...
  protected def updateClient() {
    if (
      world.getTotalWorldTime % (Configurator.detectionFrequency * 10) == searchDelay
    )
      for (i <- 0 until 6)
        if ((linkMap & 1 << i) != 0)
    ...
    if (item != null && item.getItem.isInstanceOf[ItemRouterUtility]) {
      if (!world.isRemote) {
        val s = "/#f" + "R" + getRouter.getIPAddress + " route statistics: " +
          ...
          "\n\nroute table size: " + getRouter.getRouteTable.foldLeft(0)(
            (b, v) => if (v != null) b + 1 else b
          )
  ...
  override def resolveDestination(r: NetworkPayload) {
    ...
    if (r.destinationUUID != getRouter.getID) {
      r.output =
        getRouter.getDirection(r.destinationIP, r.payload.key, r.netPriority)
      color = RouteFX2.color_relay
      if (r.output == 6) {
        var m = 0
        for (i <- 0 until 6)
          if (getStraight(i).isInstanceOf[TNetworkSubsystem]) m |= 1 << i
  ...
  override def getActiveFreeSpace(item: ItemKey) = {
    ...
    if (real == null) 0
      ...
      InvWrapper
        .wrap(real)
        .setSlotsFromSide(getInterfacedSide)
...
abstract class AbstractNetPipe
    extends PayloadPipePart[NetworkPayload]
    with TRedstonePipe
