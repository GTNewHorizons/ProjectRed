...
class RoutedInterfacePipePart
    extends AbstractNetPipe
    with TNetworkPipe
    with IWorldBroadcaster
    with IWorldCrafter
    with INeighborTileChange {
  ...
  def createContainer(player: EntityPlayer) =
    new ContainerInterfacePipe(this, player)
  ...
  def refreshChips() {
    for (i <- 0 until chipSlots.getSizeInventory) {
      ...
      if (newKey != oldKey) {
        val oldChip = chips(i)
        val newChip =
          if (newStack != null && ItemRoutingChip.isValidChip(newStack))
            ItemRoutingChip.loadChipFromItemStack(newStack)
          else null
  ...
  override def itemsToProcess =
    chips.filterNot(_ == null).foldLeft(0) { (count, r) =>
      r.getProcessingItems + count
    }
}

class ContainerInterfacePipe(pipe: RoutedInterfacePipePart, p: EntityPlayer)
