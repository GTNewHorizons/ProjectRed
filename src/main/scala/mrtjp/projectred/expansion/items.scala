        ...
        val newStack =
          if (b.isEmpty)
            new ItemStack(
              b.getChargedVariant,
              1,
              b.getChargedVariant.getMaxDamage
            )
          else stack
      ...
      case b: TItemBattery if b.nonEmpty =>
        ...
        val newStack =
          if (stack.getItemDamage >= stack.getMaxDamage)
            new ItemStack(b.getEmptyVariant)
          else stack
        (newStack, toDraw)
      case _ => (stack, 0)
    }
...
class ItemBatteryEmpty
    extends ItemCore("projectred.expansion.emptybattery")
    with TItemBattery {
...
class ItemBattery
    extends ItemCore("projectred.expansion.battery")
    with TItemBattery {
  setMaxDamage(1600)
  setNoRepair()
  setMaxStackSize(1)
...
class ItemElectronicScrewdriver
    extends ItemCore("projectred.expansion.electric_screwdriver")
    with IScrewdriver
    with IChargable {
  setMaxStackSize(1)
  setMaxDamage(400)
  ...
  override def canUse(player: EntityPlayer, stack: ItemStack) =
...
class ItemJetpack
    extends ItemArmor(ArmorMaterial.DIAMOND, 0, 1)
    with IChargable
    with ISpecialArmor {
  setMaxStackSize(1)
  setMaxDamage(6400)
  ...
  override def onArmorTick(
      world: World,
      player: EntityPlayer,
      stack: ItemStack
  ) {
    if (
      SpacebarServerTracker.isKeyDown(
        player
      ) && stack.getItemDamage < stack.getMaxDamage
    ) {
  ...
  def getPowerDraw(stack: ItemStack): Int = {
    ...
    val i = EnchantmentHelper.getEnchantmentLevel(
      ProjectRedExpansion.enchantmentFuelEfficiency.effectId,
      stack
    )
    math.max(1, 16 - i * 3)
  }
  ...
  override def getArmorModel(
      entityLiving: EntityLivingBase,
      stack: ItemStack,
      slot: Int
  ): ModelBiped = ModelJetpack

  override def damageArmor(
  ...
  def propellPlayer(player: EntityPlayer, stack: ItemStack) {
    ...
    val accelY =
      if (player.isSneaking) math.min(-velY * stabalizeSpeed, thrust * power)
      else thrust * power
    ...
    player.fallDistance =
      if (player.motionY < 0)
        ((player.motionY * player.motionY) / 0.065).toFloat
      else 0
  }

}

class EnchantmentFuelEfficiency(id: Int)
    extends Enchantment(id, 1, EnumEnchantmentType.armor_torso) {
  setName("projectred.expansion.fuel_efficiency")

  override def getMinLevel = 1
  override def getMaxLevel = 4

  override def canApply(stack: ItemStack) =
    stack.getItem.isInstanceOf[ItemJetpack]
}
...
object ItemJetpack {
  ...
  def setStateOfEntity(id: Int, state: Boolean, isServer: Boolean) {
    ...
    if (isServer && prev != isEntityUsing(id)) {
      val packet =
        new PacketCustom(ExpansionSPH.channel, ExpansionSPH.jetpack_state)
  ...
  def renderParticlesForPlayer(player: EntityPlayer) {
    val pos1 = new Vector3(-2.5 / 16d, -15 / 16d, -4 / 16d).apply(
      new Rotation(math.toRadians(-player.renderYawOffset), 0, 1, 0) `with`
        new Translation(player.posX, player.posY, player.posZ)
    )
    val pos2 = new Vector3(2.5 / 16d, -15 / 16d, -4 / 16d).apply(
      new Rotation(math.toRadians(-player.renderYawOffset), 0, 1, 0) `with`
        new Translation(player.posX, player.posY, player.posZ)
    )
    ...
    val s =
      -player.worldObj.rand.nextDouble() * 0.2 + math.min(0, player.motionY)
...
class ItemInfusedEnderPearl
    extends ItemCore("projectred.expansion.infused_ender_pearl") {
  setMaxStackSize(1)
