...
abstract class RoutingChip {
  ...
  def openGui(player: EntityPlayer) {
    ...
    GuiChipConfig.open(
      ...
      _.writeByte(player.inventory.currentItem)
    )
  }

  def createContainer(player: EntityPlayer) =
    new ContainerChipConfig(player, this)
}
...
abstract class NetworkEvent {
  ...
  def setCanceled() {
    if (!isCancelable)
      throw new Exception(
        s"Network event ${this.getClass.getSimpleName} cannot be canceled"
      )
    if (canceled)
      throw new Exception(
        s"Network event ${this.getClass.getSimpleName} is already canceled"
      )
    canceled = true
  }
...
trait TChipFilter extends RoutingChip {
  ...
  def applyFilter(
    ...
    if (enablePatterns && patterns)
      inv
        .setMatchOptions(metaMatch, nbtMatch, oreMatch)
        .setDamageGroup(grpPerc(damageGroupMode))
...
trait TChipOrientation extends RoutingChip {
  ...
  def side =
    if (extractOrient <= -1) invProvider.getInterfacedSide else extractOrient
...
trait TChipCrafter extends RoutingChip {
  ...
  def addMatrixInfo(list: ListBuffer[String]) {
    ...
    if (stack != null)
      list += (EnumChatFormatting.GRAY.toString + " - Yields: " + stack.getDisplayName + " (" + stack.stackSize + ")")
  }
...
trait TChipCrafterExtension extends RoutingChip {
  ...
  def addExtIDInfo(list: ListBuffer[String]) {
    list += EnumChatFormatting.GRAY.toString + "Extension ID: " + id.toString
      .split("-")(0) + " ..."
  }
}
