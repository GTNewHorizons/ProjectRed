...
object RenderGate {
  ...
  def spawnParticles(gate: GatePart, rand: Random) {
    renderers(gate.subID)
      .asInstanceOf[GateRenderer[GatePart]]
...
abstract class GateRenderer[T <: GatePart] {
  ...
  def spawnParticles(gate: T, rand: Random) {
    ...
    for (t <- torches) if (rand.nextInt(torches.length) == 0) {
      val pos = new Vector3(rand.nextFloat, rand.nextFloat, rand.nextFloat)
        .add(-0.5)
        .multiply(0.05, 0.1, 0.05)
...
class RenderSRLatch extends GateRenderer[SequentialGatePart] {
  ...
  val torches1 =
    Seq(new RedstoneTorchModel(8, 3, 6), new RedstoneTorchModel(8, 13, 6))
  val torches2 =
...
class RenderToggleLatch extends GateRenderer[SequentialGatePart] {
  ...
  val torches =
    Seq(new RedstoneTorchModel(4, 4, 6), new RedstoneTorchModel(4, 12, 6))
...
class RenderTimer extends GateRenderer[SequentialGatePart] {
  ...
  override def prepareDynamic(part: SequentialGatePart, frame: Float) {
    pointer.angle =
      part.getLogic[TTimerGateLogic].interpPointer(frame) * MathHelper.pi * 2
  }
...
class RenderSequencer extends GateRenderer[SequentialGatePart] {
  ...
  override def prepareDynamic(gate: SequentialGatePart, frame: Float) {
    ...
    pointer.angle =
      (gate.world.getWorldTime % max + frame) / max * 2 * MathHelper.pi
...
class RenderStateCell extends GateRenderer[SequentialGatePart] {
  ...
  val torches =
    Seq(new RedstoneTorchModel(10, 3.5, 6), new RedstoneTorchModel(13, 8, 12))
  ...
  override def prepareDynamic(part: SequentialGatePart, frame: Float) {
    reflect = part.shape == 1
    pointer.angle =
      part.getLogic[StateCell].interpPointer(frame) - MathHelper.pi / 2
  }
...
class RenderBusXcvr extends GateRenderer[BundledGatePart] {
  ...
  val panels = Seq(
    new SigLightPanelModel(4, 8, false),
    new SigLightPanelModel(12, 8, true)
  )
...
class RenderNullCell extends RenderArrayCell {
  ...
  override val bottomWire: CellBottomWireModel = new CellBottomWireModel(
    nullCellWireBottom
  )
  override val coreModels =
    Seq(bottomWire, topWire, new CellFrameModel, new NullCellBaseModel)
}
...
class RenderInvertCell extends RenderArrayCell {
  ...
  override val topWire: CellTopWireModel = new CellTopWireModel(
    extendedCellWireTop
  )
  override val bottomWire: CellBottomWireModel = new CellBottomWireModel(
    extendedCellWireBottom
  )
  override val coreModels = wires ++ Seq(
    torch,
    bottomWire,
    topWire,
    new CellFrameModel,
    new CellPlateModel,
    new ExtendedCellBaseModel
  )
...
class RenderBufferCell extends RenderArrayCell {
  ...
  val torches =
    Seq(new RedstoneTorchModel(11, 13, 6), new RedstoneTorchModel(8, 8, 6))
  ...
  override val coreModels = wires ++ torches ++ Seq(
    topWire,
    bottomWire,
    new CellFrameModel,
    new CellPlateModel,
...
class RenderSegmentDisplay extends GateRenderer[BundledGatePart] {
  ...
  override def prepareInv() {
    ...
    sevenSeg0.signal = 64
    sixteenSeg.signal = 0
    Seq(sevenSeg0, sevenSeg1, sixteenSeg).foreach(
      _.setColourOn(Colors.RED.ordinal.toByte)
    )
  }
