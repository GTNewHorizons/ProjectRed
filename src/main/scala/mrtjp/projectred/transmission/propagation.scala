...
object WirePropagator {
  ...
  def propagateTo(part: IWirePart, from: TMultiPart, mode: Int) {
    var p = currentRun
    if (p == null)
      p = if (reusableRuns.isEmpty) new PropagationRun else reusableRuns.pop
...
//            println(count+" propogations, "+partChanges.size+" part changes, "+res_NeighborChanges.size+" block updates")
    ...
    res_NeighborChanges.foreach(b =>
      world.notifyBlockOfNeighborChange(b.x, b.y, b.z, MultipartProxy.block)
    )
...
//        if (CommandDebug.WIRE_READING) println(recalcs+" recalculations")
  ...
  private def runLoop() {
    ...
    do {
      ...
      pChange =
        true // we emptied it, probably changed it, but if we didnt, the loop will break anyway.
      ptmp.foreach(_.go())

      fetch() // Update results

      if (ptmp.isEmpty && atmp.nonEmpty) {
        propagationList = analogDrops; ptmp = atmp;
        pChange = false // atmp is already up to date, so now ptmp is too.
        analogDrops = Vector.newBuilder;
        aChange = true // atmp was nonempty, now it is
      }
    } while (ptmp.nonEmpty)
    finish()
  }
  ...
  def addAnalogDrop(part: IWirePart) {
    analogDrops += new Propagation(
      part,
      WirePropagator.notApart,
      IWirePart.RISING
    )
    aChange = true
  }
...
trait IWirePart {
  ...
  /** @param side
    *   The side of this part to test for wire connection. For face parts, a
    *   rotation, or -1 for center (up). For center parts, a forgedirection.
    * @return
