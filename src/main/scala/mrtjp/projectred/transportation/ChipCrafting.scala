...
trait TActiveLostStack extends RoutingChip {
  ...
  def requestLostItems() {
    ...
    val toRequest =
      math.min(amount, routeLayer.getRequester.getActiveFreeSpace(key))
...
class ChipCrafting
    ...
    with TActiveBroadcastStack
    with TActiveLostStack {
  ...
  override def extractItem(item: ItemKey, amount: Int) = {
    ...
    if (real != null)
      InvWrapper
        .wrap(real)
        .setSlotsFromSide(invProvider.getInterfacedSide)
  ...
  override def requestPromise(
      request: RequestBranchNode,
      existingPromises: Int
  ) {
    ...
    request.addPromise(
      new DeliveryPromise(
        craftedItem.key,
        math.min(remaining, request.getMissingCount),
        routeLayer.getBroadcaster,
        true,
        true
      )
    )
  ...
  override def deliverPromise(
    ...
    addOrder(
      ItemKeyStack.get(promise.item, promise.size),
      requester,
      Priorities.ACTIVEC
    )
  }
  ...
  override def requestCraftPromise(request: RequestBranchNode) = {
    ...
    if (result != null && request.getRequestedPackage.matches(result.key)) {
      val promise =
        new CraftingPromise(result, routeLayer.getCrafter, preference)
      for (i <- 0 until 9) {
        val stack = matrix.getStackInSlot(i)
        if (stack != null && stack.stackSize > 0)
          promise.addIngredient(
            ItemKeyStack.get(stack),
            createEqualityFor(i),
            getCrafterForSlot(i)
          )
      }
  ...
  def getCrafterForSlot(i: Int): IWorldRequester = {
    ...
    if (s != null && ItemRoutingChip.hasChipInside(s)) {
      val c = ItemRoutingChip
        .loadChipFromItemStack(s)
        .asInstanceOf[TChipCrafterExtension]

      val routers = ChipCraftingExtension.getRoutersForExtension(c.id)
      if (routers.size == 1) {
        val r =
          RouterServices.getRouter(RouterServices.getIPforUUID(routers.head))
        if (
          r != null && r.isLoaded && r.isInNetwork(
            routeLayer.getRouter.getIPAddress
          )
        ) r.getParent match {
