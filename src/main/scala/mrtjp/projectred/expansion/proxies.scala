...
class ExpansionProxy_server extends IProxy with IPartFactory2 {
  ...
  def init() {
    ...
    // Enchantments
    enchantmentFuelEfficiency = new EnchantmentFuelEfficiency(
      Configurator.enchantment_fuel_efficiencty_id
    )
  }
...
class ExpansionProxy_client extends ExpansionProxy_server {
  ...
  override def postinit() {
    ...
    MinecraftForgeClient.registerItemRenderer(
      Item.getItemFromBlock(machine2),
      RenderBatteryBox
    )
...
object ForwardClientTracker extends TClientKeyTracker {
  ...
  override def getIsKeyPressed =
    Minecraft.getMinecraft.gameSettings.keyBindForward.getIsKeyPressed
}
...
object ExpansionRecipes {
  ...
  private def initItemRecipes() {
    ...
    GameRegistry.addRecipe(
      new ShapedOreRecipe(
        ...
        Items.emerald,
        'a': JC,
        new ItemStack(machine2, 1, 5)
      )
    )

    // Recipe Plan
    GameRegistry.addRecipe(
      new ShapelessOreRecipe(new ItemStack(itemPlan), "dyeBlue", Items.paper)
    )
  }
  ...
  private def initDeviceRecipes() {
    ...
    GameRegistry.addRecipe(
      new ShapedOreRecipe(
        new ItemStack(itemSolar),
        "sss",
        "iwi",
        "wew",
        's': JC,
        PartDefs.ELECTROSILICON.makeStack,
        'i': JC,
        "ingotIron",
        'e': JC,
        "ingotElectrotineAlloy",
