...
class SolarPanelPart
    extends TMultiPart
    with TFaceElectricalDevice
    with ILowLoadMachine {
  ...
  override def update() {
    ...
    if (!world.isRemote) {
      ...
      if (cond.voltage() < 100.0) {
        val I =
          2.5 * heightMultiplier * timeOfDayMultiplier * sideMultiplier * rainMultiplier * visibilityMultiplier
        cond.applyCurrent(I)
      }
    }
  }
  ...
  def visibilityMultiplier =
    if (tile.partMap(1) != null) 0.0
      ...
      world.canBlockSeeTheSky(x, y + 1, z) && !world
        .getBlock(x, y + 1, z)
        .getMaterial
        .isOpaque
    ) 0.7
