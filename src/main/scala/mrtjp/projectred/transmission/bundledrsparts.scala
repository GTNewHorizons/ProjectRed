...
trait TFaceBundledAquisitions
    extends TBundledAquisitionsCommons
    with TFaceAcquisitions {
  ...
  override def calcInternalArray(r: Int) =
    resolveArray(getInternal(r), rotFromInternal(r))

  def calcCenterArray = resolveArray(getCenter, side)
}

trait TCenterBundledAquisitions
    extends TBundledAquisitionsCommons
    with TCenterAcquisitions {
...
object BundledCommons {
  ...
  def raiseSignal(ofThis: Array[Byte], fromThat: Array[Byte]): Array[Byte] = {
    ...
    for (i <- 0 until 16)
      if ((sig1(i) & 0xff) < (fromThat(i) & 0xff)) sig1(i) = fromThat(i)
    sig1
  }

  def copySignal(signal: Array[Byte]) =
    if (signal == null) null else signal.clone()
  ...
  def unpackDigital(signal: Array[Byte], packed: Int): Array[Byte] = {
    ...
    for (i <- 0 until 16)
      sig(i) = (if ((packed & 1 << i) == 0) 0 else 255).asInstanceOf[Byte]
    sig
  }
