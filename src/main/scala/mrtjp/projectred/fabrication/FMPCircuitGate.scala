...
class CircuitGateLogic(gate: CircuitGatePart)
    extends RedstoneGateLogic[CircuitGatePart]
    with TBundledGateLogic[CircuitGatePart]
    with TComplexGateLogic[CircuitGatePart]
    with SimulatedWorldCircuit {
  val ic = new IntegratedCircuit
  ic.network = this
  ic.outputChangedDelegate = { () =>
    if (!gate.world.isRemote) scheduledTick(gate)
  }
  ...
  override def onChange(gate: CircuitGatePart) {
    var cmask = 0
    for (r <- 0 until 4)
      if ((inputMask(gate.shape) & 1 << r) != 0 && checkRSInputChange(r))
        cmask |= 1 << r
      else if (
        (bundledInputMask(gate.shape) & 1 << r) != 0 && checkBundledInputChange(
          r
        )
      ) cmask |= 1 << r
  ...
  override def scheduledTick(gate: CircuitGatePart) {
    var cmask = 0
    for (r <- 0 until 4)
      if ((outputMask(gate.shape) & 1 << r) != 0 && checkRSOutputChange(r))
        cmask |= 1 << r
      else if (
        (bundledOutputMask(
          gate.shape
        ) & 1 << r) != 0 && checkBundledOutputChange(r)
      ) cmask |= 1 << r
...
object CircuitGateLogic {
  ...
  def packIO(ri: Int, ro: Int, bi: Int, bo: Int) =
    ri | ro << 4 | bi << 8 | bo << 12
  def unpackIO(io: Int) =
    (io & 0xf, io >> 4 & 0xf, io >> 8 & 0xf, io >> 12 & 0xf)

  def packConnModes(connmodes: Array[Int]) = connmodes.foldRight(0) {
    (i, pack) => pack << 4 | i & 0xf
  }
...
class RenderCircuitGate extends GateRenderer[CircuitGatePart] {
  ...
  override def renderDynamic(t: Transformation) {
    ...
    val f = 9 / 16d * 1.0 / s
    (new Rotation(90.0.toRadians, 1, 0, 0) `with` new Translation(
      8 / 16d * 1 / f,
      2.26 / 16d,
      11.25 / 16d * 1 / f
    ) `with`
