...
class FabricationProxy_client extends FabricationProxy_server {
  ...
  override def init() {
    ...
    MinecraftForgeClient.registerItemRenderer(
      itemICBlueprint,
      ItemRenderICBlueprint
    )

    ClientRegistry.bindTileEntitySpecialRenderer(
      classOf[TileICPrinter],
      RenderICPrinterDynamic
    )
...
object FabricationRecipes {
  def initRecipes() {
      ...
      override def getCraftingResult(inv: InventoryCrafting): ItemStack = {
        for (i <- 0 until 9) {
          ...
          i match {
            case 4 =>
              if (
                stack.getItem != ProjectRedFabrication.itemICChip ||
    ...
    GameRegistry.addRecipe(
      new ShapedOreRecipe(
        new ItemStack(icBlock, 1, 1),
        "ggg",
        "oeo",
        "iwi",
        'g': JC,
        new ItemStack(Blocks.stained_glass, 1, Colors.LIGHT_BLUE.woolID),
        'o': JC,
        Blocks.obsidian,
        'e': JC,
        "gemEmerald",
    ...
    GameRegistry.addRecipe(new IRecipe {
      ...
      override def getCraftingResult(inv: InventoryCrafting): ItemStack = {
        ...
        if (
          bp != null && bp.getItem == itemICBlueprint && ItemICBlueprint
            .hasICInside(bp)
        )
          new ItemStack(itemICBlueprint)
        else null
