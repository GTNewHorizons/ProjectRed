...
class FilterChipPanel(chip: TChipFilter) extends ChipPanelNode(chip) {
  ...
  override def onAddedToParent_Impl() {
    ...
    val b = new IconButtonNode {
      override def drawButton(mouseover: Boolean) {
        ...
        drawTexturedModalRect(
          position.x,
          position.y,
          if (chip.filterExclude) 1 else 17,
          102,
          14,
          14
        )
    ...
    b.tooltipBuilder = { list =>
      list += "Filter mode"
      list += (EnumChatFormatting.GRAY + "Items are " + (if (chip.filterExclude)
                                                           "blacklisted"
                                                         else "whitelisted"))
    }
    b.clickDelegate = { () =>
    ...
    if (chip.enablePatterns) {
      ...
      b.tooltipBuilder = { list =>
        ...
        list += (EnumChatFormatting.GRAY + "Meta is " + (if (chip.metaMatch)
                                                           "checked"
                                                         else "ignored"))
      }
      b.clickDelegate = { () =>
      ...
      b.tooltipBuilder = { list =>
        list += "NBT matching"
        list += (EnumChatFormatting.GRAY + "NBT is " + (if (chip.nbtMatch)
                                                          "checked"
                                                        else "ignored"))
      }
      b.clickDelegate = { () =>
      ...
      b.tooltipBuilder = { list =>
        ...
        list += (EnumChatFormatting.GRAY + "Ore Dictionary is " + (if (
                                                                     chip.oreMatch
                                                                   ) "checked"
                                                                   else
                                                                     "ignored"))
    ...
    if (chip.enableHiding) {
      ...
      b.tooltipBuilder = { list =>
        list += "Item hiding"
        list += (EnumChatFormatting.GRAY + "Hide " + (if (chip.hideMode == 0)
                                                        "nothing"
                                                      else
                                                        chip.hide(
                                                          chip.hideMode
...
class OrientChipPanel(chip: TChipOrientation) extends ChipPanelNode(chip) {
  {
    ...
    val sideWidget = new SideSelectNode(12, 12, 30, 30) {
      override def onSideChanged(oldside: Int) {
        chip.extractOrient =
          if (sides != 0) Integer.numberOfTrailingZeros(sides) else -1
        getContainer.saveChip()
      }
    }
...
class MatrixMatchingPanel(chip: TChipMatchMatrix) extends ChipPanelNode(chip) {
...

  {
    ...
    var b = new IconButtonNode {
      override def drawButton(mouseover: Boolean) {
        ...
        GuiDraw.drawTexturedModalRect(
          position.x,
          position.y,
          if (chip.matchMeta(idx)) 49 else 65,
          118,
          14,
          14
        )
    ...
    b.tooltipBuilder = { list =>
      list += "Metadata matching"
      list += (EnumChatFormatting.GRAY + "Meta is " + (if (chip.matchMeta(idx))
                                                         "checked"
                                                       else "ignored"))
    }
    b.clickDelegate = { () =>
    ...
    b = new IconButtonNode {
      override def drawButton(mouseover: Boolean) {
        ...
        GuiDraw.drawTexturedModalRect(
          position.x,
          position.y,
          if (chip.matchNBT(idx)) 33 else 49,
          102,
          14,
          14
        )
    ...
    b.tooltipBuilder = { list =>
      list += "NBT matching"
      list += (EnumChatFormatting.GRAY + "NBT is " + (if (chip.matchNBT(idx))
                                                        "checked"
                                                      else "ignored"))
    }
    b.clickDelegate = { () =>
    ...
    b = new IconButtonNode {
      override def drawButton(mouseover: Boolean) {
        ...
        GuiDraw.drawTexturedModalRect(
          position.x,
          position.y,
          if (chip.matchOre(idx)) 81 else 97,
          118,
          14,
          14
        )
...
class ExtensionIDPanel(chip: TChipCrafterExtension)
    extends ChipPanelNode(chip) {
  size = Size(120, 55)
  ...
  override def drawBack_Impl(mouse: Point, rframe: Float) {
    ...
    val groups =
      Seq(sec(0) + "-" + sec(1) + "-" + sec(2) + "-" + sec(3) + "-", sec(4))
