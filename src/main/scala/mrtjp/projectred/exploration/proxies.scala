...
class ExplorationProxy_server extends IProxy {
  ...
  override def init() {
    ...
    for (o <- OreDefs.values)
      blockOres.setHarvestLevel("pickaxe", o.harvest, o.meta)

    blockDecoratives = new BlockDecoratives
    for (b <- DecorativeStoneDefs.values)
      blockDecoratives.setHarvestLevel("pickaxe", b.harvest, b.meta)
    ...
    toolMaterialRuby =
      EnumHelper.addToolMaterial("RUBY", 2, 512, 8.00f, 3.00f, 10)
    toolMaterialSapphire =
      EnumHelper.addToolMaterial("SAPPHIRE", 2, 512, 8.00f, 3.00f, 10)
    toolMaterialPeridot =
      EnumHelper.addToolMaterial("PERIDOT", 2, 512, 7.75f, 2.75f, 14)

    armorMatrialRuby =
      EnumHelper.addArmorMaterial("RUBY", 16, Array(3, 8, 6, 3), 8)
    armorMatrialSapphire =
      EnumHelper.addArmorMaterial("SAPPHIRE", 16, Array(3, 8, 6, 3), 8)
    armorMatrialPeridot =
      EnumHelper.addArmorMaterial("PERIDOT", 14, Array(3, 8, 6, 3), 10)
    ...
    if (Configurator.gen_MarbleCave) {
      ...
      gen.cluster = Set(
        ((blockDecoratives, DecorativeStoneDefs.MARBLE.meta), 1)
      )
    ...
    if (Configurator.gen_Volcano) {
      ...
      gen.ashCluster = Set(
        ((blockDecoratives, DecorativeStoneDefs.BASALT.meta), 1)
      )
