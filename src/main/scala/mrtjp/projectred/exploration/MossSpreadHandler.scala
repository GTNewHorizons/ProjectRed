...
object MossSpreadHandler extends IBlockEventHandler {
  ...
  private def isBlockTouchingAir(w: World, b: BlockCoord) = ncheck(w, b) {
    _ == Blocks.air
  }

  private def isBlockWet(w: World, b: BlockCoord) = ncheck(w, b) { b =>
    b == Blocks.flowing_water || b == Blocks.water
  }

  private def isBlockHot(w: World, b: BlockCoord) = ncheck(w, b) { b =>
    b == Blocks.flowing_lava || b == Blocks.lava
  }
