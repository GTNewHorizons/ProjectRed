...
trait TRedstonePipe
    extends SubcorePipePart
    with TCenterRSAcquisitions
    with TCenterRSPropagation
    with IRedwirePart
  ...
  override def getDrops = if (material)
    super.getDrops :+ getMaterialStack
  else super.getDrops

  def getMaterialStack =
    ItemMicroPart.create(
      769,
      BlockMicroMaterial.materialKey(Blocks.redstone_block, 0)
    )
  ...
  abstract override def canConnectPart(part: IConnectable, s: Int) =
    part match {
  ...
  override def discoverStraightOverride(absDir: Int) = {
    if (material) {
      ...
      val b = (RedstoneInteractions.otherConnectionMask(
        world,
        x,
        y,
        z,
  ...
  abstract override def activate(
      player: EntityPlayer,
    ...
    if (
      held != null && !material && held.getItem == MicroblockProxy.itemMicro && held.getItemDamage == 769
    ) {
        ...
        case bm: BlockMicroMaterial if bm.block == Blocks.redstone_block =>
          if (!world.isRemote) {
            ...
            world.playSoundEffect(
              ...
              y + 0.5,
              z + 0.5,
              Block.soundTypeGlass.func_150496_b(),
              Block.soundTypeGlass.getVolume * 5.0f,
              Block.soundTypeGlass.getPitch * 0.9f
            )
            sendMatUpdate()
    ...
    player.addChatComponentMessage(
      new ChatComponentText(
        (if (world.isRemote) "Client"
         else "Server") + " signal strength: " + getSignal
      )
    )
    true
  }
  ...
  def test(player: EntityPlayer) = {
    if (world.isRemote)
      Messenger.addMessage(x, y + .5f, z, "/#f/#c[c] = " + getSignal)
    else {
...
trait TColourFilterPipe extends SubcorePipePart {
  ...
  def getColourStack =
    if (colour == -1) null
      ...
      ItemMicroPart.create(
        769,
        BlockMicroMaterial.materialKey(Blocks.wool, colour)
      )
  ...
  abstract override def activate(
    ...
    if (
      held != null && held.getItem == MicroblockProxy.itemMicro && held.getItemDamage == 769
    ) {
      ItemMicroPart.getMaterial(held) match {
        case bm: BlockMicroMaterial
            if bm.block == Blocks.wool && bm.meta != colour =>
          if (!world.isRemote) {
            dropMaterial()
            ...
            world.playSoundEffect(
              ...
              y + 0.5,
              z + 0.5,
              bm.getSound.func_150496_b(),
              bm.getSound.getVolume * 5.0f,
              bm.getSound.getPitch * 0.9f
            )
            sendColourUpdate()
            if (!player.capabilities.isCreativeMode) held.stackSize -= 1
          }
...
trait TInventoryPipe[T <: AbstractPipePayload]
    extends PayloadPipePart[T]
    with IInventoryProvider {
  ...
  abstract override def discoverStraightOverride(s: Int): Boolean = {
    WorldLib.getTileEntity(world, posOfStraight(s)) match {
      case sinv: ISidedInventory =>
        sinv.getAccessibleSlotsFromSide(s ^ 1).nonEmpty
  ...
  override def getInventory = {
    if (0 until 6 contains inOutSide)
      InvWrapper.getInventory(world, new BlockCoord(tile).offset(inOutSide))
    else null
  }

  override def getInterfacedSide =
    if (!(0 to 5 contains inOutSide)) -1 else inOutSide ^ 1
  ...
  abstract override def activate(
    ...
    if (
      held != null && held.getItem.isInstanceOf[IScrewdriver] && held.getItem
        .asInstanceOf[IScrewdriver]
        .canUse(player, held)
    ) {
      if (!world.isRemote) {
