...
class TileICPrinter extends TileICMachine with TInventory {
  ...
  override def isItemValidForSlot(slot: Int, item: ItemStack) = {
    if (slot == 18)
      item.getItem.isInstanceOf[ItemICBlueprint] && ItemICBlueprint.hasICInside(
        item
      )
  ...
  override def update() {
    ...
    if (
      !isWorking && world.getTotalWorldTime % 10 == 0 && canStart
    ) // delay check, can be expensive
      doStart()
  }
  ...
  def canStart = {
    if (world.getTotalWorldTime % 20 == 0)
      ...
      checkOutputClear && checkInputIC && checkBlueprint && checkIngredients() // use cheaper checks as fail-fast
  }
...
class ContainerPrinter(player: EntityPlayer, tile: TileICPrinter)
    extends NodeContainer {
  {
    ...
    addSlotToContainer({
      val s = new Slot3(tile, 18, 63, 20); s.slotLimitCalculator = { () => 1 };
      s
    })
    addSlotToContainer({
      val s = new Slot3(tile, 19, 63, 46); s.slotLimitCalculator = { () => 1 };
      s
    })
    addSlotToContainer({
      val s = new Slot3(tile, 20, 134, 33); s.slotLimitCalculator = { () => 1 };
      s
    })
  ...
  override def doMerge(stack: ItemStack, from: Int): Boolean = {
    if (from == 20) {
      ...
      stack.getItem match {
        case i: ItemICBlueprint =>
          if (tryMergeItemStack(stack, 18, 19, false)) return true
        case i: ItemICChip =>
          if (tryMergeItemStack(stack, 19, 20, false)) return true
        case _ =>
      }
...
class GuiICPrinter(c: ContainerPrinter, tile: TileICPrinter)
    extends NodeGui(c, 176, 201) {
...

  {
    ...
    list.displayNodeFactory = { stack =>
      ...
      d.zPosition = -0.01
      d.backgroundColour =
        if (tile.containsEnoughOf(stack))
          Colors.LIME.rgb | 0x44000000
        else Colors.RED.rgb | 0x44000000
      d
    }
    pan.addChild(list)
...
object RenderICPrinter extends TInstancedBlockRender {
  val lowerBoxes = {
    ...
    val box = CCModel
      .quadModel(24)
      .generateBlock(0, new Cuboid6(0, 0, 0, 1, 10 / 16d, 1))
    for (r <- 0 until 4) {
  ...
  override def renderWorldBlock(
    ...
    /** However, using only the first model (with a 0 rotation, so no rotation
      * at all) and rendering it with the rotation in line, the model does not
      * shade properly. The light shading rotates WITH the model. Comment the
      * other one and uncomment this, place a IC Printer block, and rotate it
  ...
  override def registerIcons(reg: IIconRegister) {
    def register(t: String) =
      reg.registerIcon("projectred:fabrication/printer/" + t)
...
object RenderICPrinterDynamic extends TileEntitySpecialRenderer {
  ...
  def getMods = {
    val map = CCModel
      .parseObjModels(
        new ResourceLocation("projectred:textures/obj/fabrication/printer.obj"),
        7,
        null
      )
      .toMap
  ...
  def renderICChip(t: Transformation) {
    ...
    icChip.render(
      Rotation.quarterRotations(2) `with` new Translation(
        0.5,
        9.5 / 16d,
        0.5
      ) `with` t,
