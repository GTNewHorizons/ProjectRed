...
object LSPathFinder {
  ...
  private def iterate(
      ...
      coll: MBuilder[StartEndPath, Vector[StartEndPath]] =
        Vector.newBuilder[StartEndPath]
  ): Vector[StartEndPath] = open match {
    case _ if closed.size > Configurator.maxDetectionCount => coll.result()
    case Seq()                                             => coll.result()
    case Seq(next, rest @ _*) =>
      getPipe(next.bc) match {
        case iwr: IWorldRouter with TNetworkPipe if {
              val r = iwr.getRouter; r != null && r.isLoaded
            } =>
          iterate(
            rest,
            closed + next,
            coll += new StartEndPath(
              start.getRouter,
              iwr.getRouter,
              next.hop,
              next.dist,
              next.filters + iwr.pathFilter,
              iwr.networkFilter
            )
          )
        case p: TNetworkSubsystem =>
          ...
          for (s <- 0 until 6) if (s != (next.dir ^ 1) && p.maskConnects(s)) {
            val route =
              next --> (s, p.getPathWeight, p.pathFilter(next.dir ^ 1, s))
  ...
  private object Node {
    def apply(bc: BlockCoord, dir: Int): Node =
      Node(bc.copy().offset(dir), 1, dir, dir)
  }
  private case class Node(
    ...
    def -->(to: Node, toDir: Int): Node =
      Node(to.bc, dist + to.dist, toDir, hop)

    def -->(toDir: Int, distAway: Int, filter: PathFilter): Node =
      Node(bc.copy.offset(toDir), dist + distAway, toDir, hop, filters + filter)
    def -->(toDir: Int, distAway: Int): Node =
      Node(bc.copy().offset(toDir), dist + distAway, toDir, hop, filters)
    def -->(toDir: Int): Node = this --> (toDir, 1)

    def -->(to: BlockCoord, distAway: Int, filter: PathFilter): Node =
      Node(to, dist + distAway, dir, hop, filters + filter)
    def -->(to: BlockCoord, distAway: Int): Node =
      Node(to, dist + distAway, dir, hop, filters)
  ...
  def result(): Map[ItemKey, Int] = {
    ...
    for (p <- start.getRouter.getRoutesByCost) {
      ...
      if (
        parent.isInstanceOf[
          IWorldCrafter
        ] && collectCrafts && p.flagRouteFrom && p.allowCrafting
      ) {
      ...
      if (
        parent.isInstanceOf[
          IWorldBroadcaster
        ] && collectBroadcasts && p.flagRouteFrom && p.allowBroadcast
      )
...
object LogisticPathFinder {
  ...
  def sharesInventory(
      pipe1: TInventoryPipe[_],
      pipe2: TInventoryPipe[_]
  ): Boolean = {
...
class SyncResponse {
  ...
  def isPreferredOver(that: SyncResponse) =
    SyncResponse.isPreferredOver(priority.ordinal, customPriority, that)
}
