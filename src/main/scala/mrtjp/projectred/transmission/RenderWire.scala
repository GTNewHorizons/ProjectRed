...
object RenderWire {
  ...
  /** Returns a tightly packed unique index for the specific model represented
    ...
    * in that direction. 00 = none 01 = corner 10 = straight 11 = internal The
    * second byte contains the thickness*6+side
    *
    * @param side
    *   The side the wire is attached to
    * @param thickness
    *   The thickness of the wire -1 in 1/8th blocks. Supported values 0, 1, 2
    * @param connMap
    *   The connection mask of the wire
    */
  ...
  def renderBreakingOverlay(icon: IIcon, wire: WirePart) {
    ...
    boxes += new Cuboid6(0.5 - w, 0, 0.5 - w, 0.5 + w, h, 0.5 + w)
    ...
    for (r <- 0 until 4) {
      val length =
        if (connCount == 0) {
          ...
          if (connMask == (1 << (r + 2) % 4))
            4 // this side is opposite the one with a connection
          else if (connMask == (1 << r)) 8
          else 0
        } else if ((connMask & 1 << r) != 0) 8
        else 0

      if (length > 0) {
        val l = length / 16d
        boxes += new Cuboid6(0.5 - w, 0, 0.5 + w, 0.5 + w, h, 0.5 + l)
...
class WireModelGen {
  ...
  private def generateCenter() {
    var tex = connCount match // 0 = straight n/s, 1 = straight e/w, 2 = circle
    {
      ...
      case 1 =>
        if ((connMask & 5) != 0) 0
          ...
          1 // if there is one connection, and it is north/south then north/south, otherwise east/west
    ...
    if (r != 0) {
      val uvt = new UVT(
        Rotation.quarterRotations(r % 4).at(new Vector3(8, 0, 16))
      )
    ...
    val verts =
      if (isInventoryModel) generateSideInventoryModel(r)
        ...
        connCount match {
          ...
          case 1 if connMask == (1 << (r + 2) % 4) =>
            generateStub(r) // this side is opposite the one with a connection
  ...
  private def generateStub(r: Int): Array[Vertex5] = {
    ...
    for (i <- 0 until 4)
      verts(
        i
      ).vec.z -= 0.002 // pull the stub in a little so it doesn't z fight with framed cables
    reflectSide(verts, r)
    verts
  }
  ...
  /** Returns a copy of vertices with the bottom face added at the start.
    *
    * @param start
    *   The index of the first vertex making up the top face
    * @param count
    *   The number of vertices making up the top face
    */
  private def withBottom(
      verts: Array[Vertex5],
      start: Int,
      count: Int
  ): Array[Vertex5] = {
