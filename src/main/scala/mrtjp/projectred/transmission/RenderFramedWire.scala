...
object RenderFramedWire {
  ...
  private def modelKey(thickness: Int, connMap: Int): Int =
    connMap | thickness << 6
  private def modelKey(w: FramedWirePart): Int =
    modelKey(w.getThickness, w.connMap)
  ...
  def renderInv(thickness: Int, hue: Int, ops: IVertexOperation*) {
    getOrGenerateWireModel(modelKey(thickness, 0x3f)).render(
...
private object FWireModelGen {
    ...
    val uvt = rot match {
      ...
      case 2 =>
        new UVT(
          Rotation
            .quarterRotations(1)
            .at(new Vector3(8, 0, 16))
            .`with`(new Translation(16, 0, 0))
        )
      case _ => null
    }
  ...
  private def generateJacketedBoxes: Array[IndexedCuboid6] = {
    if (connCount == 0)
      return Array(new IndexedCuboid6(0, WireBoxes.fOBounds(6)))
  ...
  private def generateAxialJacketBoxes(
      a: Int,
      first: Boolean,
      boxes: Array[IndexedCuboid6]
  ): Boolean = {
...
object JacketedHighlightRenderer extends IMicroHighlightRenderer {
  override def renderHighlight(
    ...
    val tile = PRLib.getMultipartTile(
      player.worldObj,
      new BlockCoord(hit.blockX, hit.blockY, hit.blockZ)
    )
    if (
      tile == null || mcrClass.getClassId != 0 || size != 1 || player.isSneaking ||
      MicroMaterialRegistry.getMaterial(material).isTransparent
    ) false
    else {
