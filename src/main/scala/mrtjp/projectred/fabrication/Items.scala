...
object ItemICBlueprint {
  ...
  def saveICToGate(ic: IntegratedCircuit, gate: ItemStack) {
    ...
    val ioparts = ic.parts.values.collect { case io: TIOCircuitPart =>
      io
    }.toSeq
    ...
    for (r <- 0 until 4) {
      ...
      val ioMode =
        if (sparts.exists(_.getIOMode == InOut)) InOut
        else {
      ...
      val connMode =
        if (sparts.exists(_.getConnMode == Simple)) Simple
...
object ItemRenderICBlueprint extends IItemRenderer {
  override def handleRenderType(item: ItemStack, t: ItemRenderType) =
    t == ItemRenderType.FIRST_PERSON_MAP
