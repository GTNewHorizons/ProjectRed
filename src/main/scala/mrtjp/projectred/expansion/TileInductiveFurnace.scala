...
class TileInductiveFurnace extends TileProcessingMachine {
  ...
  override def canStart: Boolean = {
    ...
    val room = InvWrapper
      ...
      .getSpaceForItem(ItemKey.get(stack))
...
class ContainerFurnace(p: EntityPlayer, tile: TileInductiveFurnace)
    extends ContainerProcessingMachine(tile) {
  {
...
class GuiInductiveFurnace(tile: TileInductiveFurnace, c: Container)
  ...
  override def buildGui(player: EntityPlayer, data: MCDataInput) = {
    WorldLib.getTileEntity(player.worldObj, data.readCoord()) match {
      case t: TileInductiveFurnace =>
        new GuiInductiveFurnace(t, t.createContainer(player))
      case _ => null
    }
  }
}
