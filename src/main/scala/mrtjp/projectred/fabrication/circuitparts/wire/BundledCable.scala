...
class BundledCableICPart
  ...
  def getUpdateMask(from: CircuitPart, mode: Int) = from match {
    ...
    case b: IBundledCableICPart if mode == RISING =>
      ...
      val osignal = b.getBundledSignal
      for (i <- 0 until 16)
        if ((osignal(i) & 0xff) > (getBundledSignal.apply(i) & 0xff))
          m |= 1 << i
      m
    case _ => 0xffff
  }
  ...
  override def resolveArray(part: Any, r: Int) = {
    part match {
      case b: IBundledCableICPart =>
        ...
        for (i <- 0 until 16)
          if ((osig(i) & 0xff) - 1 > (tmpSignal(i) & 0xff))
  ...
  override def propagateTo(part: CircuitPart, mode: Int) = {
    def shouldPropogate(part: CircuitPart, mode: Int) = part match {
      case ins: IInsulatedRedwireICPart =>
        (propagatingMask & 1 << ins.getInsulatedColour) != 0
      case _ => true
    }
  ...
  override def getBundledSignal(r: Int) =
    if (maskConnects(r)) getBundledSignal else null
  ...
  override def getRolloverData(detailLevel: Int) = {
    ...
    if (detailLevel >= 3) data += GRAY + "signal: 0x" + Integer.toHexString(sig)
      ...
      data += GRAY + "state: " + (if (sig != 0) "active" else "inactive")
...
object RenderICBundledCable {
  ...
  def render(t: Transformation, ortho: Boolean) {
    ...
    faceModels(dynamicIdx(0, ortho))
      .render(t, new IconTransformation(bundledwireIcons(connMap & 0xff)))
    if (colour != -1)
      faceModels(dynamicIdx(0, ortho)).render(
        t,
        new IconTransformation(bundledColourIcon),
