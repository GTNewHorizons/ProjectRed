...
class TileChargingBench
  ...
  override def createContainer(player: EntityPlayer) =
    new ContainerChargingBench(player, this)
  ...
  def dropStackDown(stack: ItemStack) = {
    val wr =
      InvWrapper.wrap(this).setSlotsFromRange(8 until 16).setInternalMode(true)
...
class ContainerChargingBench(p: EntityPlayer, tile: TileChargingBench)
...
class GuiChargingBench(tile: TileChargingBench, c: ContainerChargingBench)
    extends NodeGui(c, 176, 183) {
  override def drawBack_Impl(mouse: Point, frame: Float) {
    ...
    if (
      tile.cond.charge > tile.getDrawCeil && tile.storage < tile.getMaxStorage
    )
...
object GuiChargingBench extends TGuiBuilder {
  ...
  override def buildGui(player: EntityPlayer, data: MCDataInput) = {
    WorldLib.getTileEntity(player.worldObj, data.readCoord()) match {
      case t: TileChargingBench =>
        new GuiChargingBench(t, t.createContainer(player))
      case _ => null
    }
  }
}
