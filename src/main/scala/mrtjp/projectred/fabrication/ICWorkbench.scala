...
class BlockICMachine
    extends InstancedBlock("projectred.integration.icblock", Material.iron) {
  setHardness(2)
...
abstract class TileICMachine extends InstancedBlockTile with TTileOrient {
  ...
  override def onBlockActivated(player: EntityPlayer, side: Int): Boolean = {
    ...
    if (
      doesRotate && held != null && held.getItem
        .isInstanceOf[IScrewdriver] && held.getItem
...
class TileICWorkbench extends TileICMachine with NetWorldCircuit {
  ...
  override def onBlockActivated(player: EntityPlayer, side: Int): Boolean = {
    ...
    if (!world.isRemote) {
      ...
      if (
        !hasBP && player.getHeldItem != null && player.getHeldItem.getItem
          .isInstanceOf[ItemICBlueprint]
      ) {
        ...
        val item =
          new EntityItem(world, p.x + 0.5, p.y + 0.20, p.z + 0.5, stack)
...
object RenderICWorkbench extends TInstancedBlockRender with TCubeMapRender {
  ...
  override def registerIcons(reg: IIconRegister) {
    def register(s: String) =
      reg.registerIcon("projectred:fabrication/icworkbench/" + s)
    ...
    iconTBP = new MultiIconTransformation(
      bottom,
      topBP,
      sidebp1,
      sidebp1,
