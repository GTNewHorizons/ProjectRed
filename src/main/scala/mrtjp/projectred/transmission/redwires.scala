...
trait TRedwireCommons
    extends TWireCommons
    with TRSAcquisitionsCommons
    with TRSPropagationCommons
    with IRedwirePart {
  var signal: Byte = 0
  ...
  override def test(player: EntityPlayer) = {
    if (world.isRemote)
      Messenger.addMessage(x, y + 0.5, z, "/#f/#c[c] = " + getSignal)
    else {
...
abstract class RedwirePart
    extends WirePart
    with TRedwireCommons
    with TFaceRSAcquisitions
    with TFaceRSPropagation {
  override def weakPowerLevel(side: Int) = {
    if (
      (side & 6) != (this.side & 6) && (connMap & 0x100 << Rotation.rotationTo(
        this.side,
        side
      )) != 0
    ) 0
...
abstract class FramedRedwirePart
  ...
  override def canConnectPart(part: IConnectable, s: Int) =
    part.isInstanceOf[IRedwirePart]
  ...
  override def calcInternalSignal(s: Int) = {
    ...
    if (sig > 0) sig
      ...
      tp match {
        case rp: IRedstonePart =>
          Math.max(rp.strongPowerLevel(s ^ 1), rp.weakPowerLevel(s ^ 1)) << 4
        case _ => 0
      }
  }
...
class RedAlloyWirePart extends RedwirePart with TRedAlloyCommons {
  override def strongPowerLevel(side: Int) =
    if (side == this.side) rsLevel else 0
  ...
  override def propagateOther(mode: Int) {
    ...
    for (r <- 0 until 4)
      if (!maskConnects(r))
        WirePropagator.addNeighborChange(
          new BlockCoord(tile).offset(Rotation.rotateSide(side, r))
        )

    for (s <- 0 until 6)
      if (s != (side ^ 1))
        WirePropagator.addNeighborChange(
          new BlockCoord(tile).offset(side).offset(s)
        )
  }
}
...
trait TInsulatedCommons extends TRedwireCommons with IInsulatedRedwirePart {
  ...
  abstract override def canConnectPart(part: IConnectable, r: Int) =
    part match {
...
class InsulatedRedAlloyPart extends RedwirePart with TInsulatedCommons {
  override def weakPowerLevel(side: Int) = {
    if (
      this.side == side || this.side == (side ^ 1) || !maskConnects(
        absoluteRot(side)
      )
    ) 0
...
class FramedInsulatedRedAlloyPart
    extends FramedRedwirePart
    with TInsulatedCommons {
  override def weakPowerLevel(side: Int) = {
