...
class BlockBarrel
    extends InstancedBlock("projectred.exploration.barrel", Material.wood) {
  setHardness(2.0f)
...
class TileBarrel
  ...
  override def onBlockActivated(player: EntityPlayer, side: Int): Boolean = {
    ...
    if (
      (checkDoubleClick() && importAll(player) > 0) ||
      importStack(player.getHeldItem) > 0
    ) needsUpdate = true
  ...
  override def onBlockClicked(player: EntityPlayer): Boolean = {
    ...
    val toRem = math.min(
      if (player.isSneaking) 1
        ...
        inslot.stackSize - (if (stored > 1 && stored <= inslot.getMaxStackSize)
                              1
                            else 0),
  ...
  def compactItems() {
    ...
    if (getStackInSlot(0) != null) {
      ...
      if (out == null || !InvWrapper.areItemsSame(in, out))
        out = in.splitStack(0)
...
class BarrelInvWrapper(inv: IInventory) extends InvWrapper(inv) {
  ...
  override def getItemCount(item: ItemKey) = {
    var count =
      if (
        slots
          .contains(1) && getBarrel.nonEmpty && eq.matches(item, getBarrel.item)
      ) getBarrel.getStoredAmount
      else 0
  ...
  override def extractItem(item: ItemKey, toExtract: Int) = {
    ...
    if (slots.contains(1) && eq.matches(item, getBarrel.item)) {
      while (itemsLeft > 0 && getBarrel.getStoredAmount - hidden > 0) {
        ...
        if (bottomStack.stackSize <= 0)
          getBarrel.setInventorySlotContents(1, null)
  ...
  override def getAllItemStacks: Map[ItemKey, Int] = {
    if (slots.contains(1) && getBarrel.nonEmpty) {
      val count =
        getBarrel.getStoredAmount - (if (hidePerSlot || hidePerType) 1 else 0)
