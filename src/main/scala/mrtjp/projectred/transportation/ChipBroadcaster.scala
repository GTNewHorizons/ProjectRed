...
trait TActiveBroadcastStack extends RoutingChip {
  ...
  def getDeliveryCount(item: ItemKey) = orders.foldLeft(0)((b, p) =>
    b + (if (p.stack.key == item) p.stack.stackSize else 0)
  )
...
class ChipBroadcaster
    extends RoutingChip
    with TChipFilter
    with TChipOrientation
    with TChipPriority
  ...
  override def requestPromise(
    ...
    for (
      (key, amount) <- inv.getAllItemStacks.filter { p =>
        requested.matches(p._1) && filt.hasItem(p._1) != filterExclude
      }
    ) {
      val available = amount - request.root.getExistingPromisesFor(
