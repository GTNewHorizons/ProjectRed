...
import mrtjp.projectred.fabrication.circuitparts.io.{AnalogIOICGateLogic, IOGateICPart}

import scala.util.Try

class ConfigurationAnalogIO(gate: IOGateICPart) extends ConfigurationSimpleIO(gate) {
  ...
  override def drawBack_Impl(mouse: Point, rframe: Float): Unit = {
    ...
    GuiDraw.drawString("Frequency:", pos_text.x, pos_text.y, Colors.GREY.argb, false)
  }
}
