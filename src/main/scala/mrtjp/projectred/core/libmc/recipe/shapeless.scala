...
class ShapelessRecipeBuilder
    extends RecipeBuilder // with TMappedRecipeBuilder
    {
  def result() = {
    compute()
...
class ShapelessBuilderRecipe(val builder: ShapelessRecipeBuilder)
    extends IRecipe {
  ...
  override def loadCraftingRecipes(result: ItemStack) {
    for (
      ...
      case r: ShapelessBuilderRecipe
          if NEIServerUtils
            .areStacksSameTypeCrafting(irecipe.getRecipeOutput, result) =>
