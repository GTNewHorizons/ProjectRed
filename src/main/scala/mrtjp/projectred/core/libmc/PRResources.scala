...
object PRResources {
  ...
  val guiScrollBar = registerPR("textures/gui/scrollbar.png")
  ...
  val guiElectrotineGenerator = registerPR(
    "textures/gui/electrotine_generator.png"
  )
  val guiCharger = registerPR("textures/gui/charger.png")

  def register(path: String) = new ResourceAction(new ResourceLocation(path))
  def registerPR(path: String) = new ResourceAction(
    new ResourceLocation("projectred", path)
  )
}
